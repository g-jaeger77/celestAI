...
def resolve_executable(cmd_name):
    """Resolve executable name for Windows."""
    if sys.platform == 'win32':
        # Try finding the command as is
        ...
        for ext in ['.cmd', '.exe', '.ps1']:
            ...
            if path:
                return path
        # Fallback for npm/npx if not found in path but assumed to be there
        if cmd_name in ['npm', 'npx']:
             return f"{cmd_name}.cmd"
    return cmd_name
...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            if "lint" in scripts:
                result["linters"].append({"name": "npm lint", "cmd": [resolve_executable("npm"), "run", "lint"]})
            elif "eslint" in deps:
                result["linters"].append({"name": "eslint", "cmd": [resolve_executable("npx"), "eslint", "."]})
            
            # Check for TypeScript
            if "typescript" in deps or (project_path / "tsconfig.json").exists():
                result["linters"].append({"name": "tsc", "cmd": [resolve_executable("npx"), "tsc", "--noEmit"]})
